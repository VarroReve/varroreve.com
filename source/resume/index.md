...
你好，我是王隆，一名生于 1994 年，具备四年工作经验的软件工程师。这是我的 [在线简历](https://varroreve.com/resume)，以及 [PDF 版本](https://s3.amazonaws.com/insta.cloud/resume.pdf)（由于我会经常更新简历，推荐使用在线版本）；这是我的 [联系方式](#联系方式)。
...
- **为 firmoo.com 建设国际化分站**
  ...
  最终，firmoo 通过此方案上线了英国、西班牙、澳大利亚、法国、德国等分站。

- **常规运维**

  - 负责 AWS 云服务产品接入与使用
  - 负责 PHP、SQL 相关的优化及故障排查
  - 安装常见服务端软件，维护 EC2 服务器运行
  - 搭建能承载公司上百人使用的科学上网服务
...
公司的主营产品转单宝是一个链接鲜花、蛋糕、礼品行业商家之间的订单交易平台，通过接入第三方电商平台（淘宝、京东、美团等）做到统一管理、快速转派订单，解决商家卖货难、无货、多平台商品管理成本高等问题。

我负责消息推送及搜索的微服务模块。推送服务使用消息模板管理推送消息，支持极光、微信、短信、websocket 推送渠道，切换不同推送策略以达到分区管理。搜索项目使用 ElasticSearch 对店铺、订单、日志的文档进行分布式存储及索引。

推送、搜索模块介绍：

- 使用 Laravel 作为微服务框架，MongoDB 作为主数据库，Redis 作为队列服务。
...
API 项目负责游戏数据接口，在重构前使用 PHP-FPM 同步阻塞方式处理请求，已无法承受更多并发；使用 异步协程框架 Swoole 重构后，通过协程、异步非阻塞查询，数据库链接池等方案，接口的响应速度、并发量得到显著提升。

游戏后台、代理商后台由 TP2.0、TP3.2 框架，重构为 Laravel 框架。负责游戏配置、代理商推广。
